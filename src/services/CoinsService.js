...
class CoinsService {
    ...
    static async synchroniser(token) {
        ...
        // Envoyer au serveur (simulé ici, à adapter selon l'API backend)
        // Dans une implémentation réelle, on ferait un POST /api/transactions/sync
        
        // Pour l'instant, on marque juste comme synchronisé
        await TransactionService.markAsSynced(pending.map(t => t.id));
    }
}
